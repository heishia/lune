...
export function PrivacyPolicyModal({ isOpen, onClose }: PrivacyPolicyModalProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* 헤더 */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-brand-warm-taupe/20">
          <h2 className="text-lg font-medium text-brand-terra-cotta">개인정보처리방침</h2>
          <button
            onClick={onClose}
            className="p-1 hover:bg-brand-warm-taupe/10 rounded-md transition-colors"
          >
            <X className="w-5 h-5 text-brand-terra-cotta" />
          </button>
        </div>

        {/* 본문 */}
        <div className="flex-1 overflow-y-auto px-6 py-4 text-sm text-black/80 leading-relaxed">
        ...
        {/* 푸터 */}
        <div className="px-6 py-4 border-t border-brand-warm-taupe/20">
          <Button
            onClick={onClose}
            className="w-full bg-brand-terra-cotta text-white hover:bg-brand-warm-taupe h-12"
          >
            확인
          </Button>
        </div>
